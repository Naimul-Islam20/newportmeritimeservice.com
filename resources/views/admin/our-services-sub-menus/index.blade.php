@extends('layouts.admin', ['title' => 'Our Services — menu'])

@section('content')
<div class="header">
    <h1>Our Services — sub menus</h1>
    @can('create', \App\Models\SubMenu::class)
    <a href="{{ route('admin.our-services-sub-menus.create') }}" class="btn btn-primary">Create</a>
    @endcan
</div>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="actions-cell">
                        @can('update', $sub)
                        <form method="POST" action="{{ route('admin.our-services-sub-menus.toggle-active', $sub) }}" style="display:inline;">
                            @csrf
                            @method('PATCH')
                        ...
                        <a class="btn btn-muted" href="{{ route('admin.our-services-sub-menus.edit', $sub) }}">Edit</a>
                        @if (! $sub->isFormPageLink())
                        <a class="btn btn-primary" href="{{ $sub->adminSidebarHref() }}">Page content</a>
                        @endif
                        @endcan
                        @can('delete', $sub)
                        <form method="POST" action="{{ route('admin.our-services-sub-menus.destroy', $sub) }}" style="display:inline;">
                            @csrf
                            @method('DELETE')
                ...
                @empty
                <tr>
                    <td colspan="6">No sub-menus yet. <a href="{{ route('admin.our-services-sub-menus.create') }}">Create one</a>.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
