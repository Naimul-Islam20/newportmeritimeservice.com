...
@php
    ...
    $left = is_array($section->left_content ?? null) ? $section->left_content : [];
    $right = is_array($section->right_content ?? null) ? $section->right_content : [];
    $typeLabels = [
        'image' => 'Image',
...
<div class="card" style="margin-bottom:12px; color:#64748b; font-size:13px;">
    Section type: <strong>{{ $typeLabels[$formType] ?? $formType }}</strong>
</div>
...
<div class="card">
    <form method="POST" action="{{ route('admin.home-sections.update', $section) }}" enctype="multipart/form-data">
        ...
        @if ($formType === 'two_column_image_details')
            <input type="hidden" name="image_side" id="tci_image_side" value="{{ old('image_side', data_get($d, 'image_side', 'left')) }}">
            <input type="hidden" name="layout_width" id="tci_layout_width" value="{{ old('layout_width', $section->layout_width ?: 'full') }}">
            ...
            <div style="display:grid; grid-template-columns: 1fr 1fr; gap:14px; margin-top:12px;">
                ...
                <div id="tciDetailsCol" style="order: 1; border:1px solid #e5e7eb; border-radius:10px; background:#fff; padding:14px;">
                    ...
                    <div style="margin-top:10px;">
                        <label>Point</label>
                        @php($pts = is_array(old('points', $section->points ?? [])) ? old('points', $section->points ?? []) : [])
        ...
        @elseif ($formType === 'two_column_two_side_details')
            <div class="grid grid-2" style="margin-top:12px;">
                ...
                <div>
                    ...
                    <input id="left_title" name="left_title" value="{{ old('left_title', $left['title'] ?? '') }}" placeholder="Optional">
                </div>
                <div>
                    <label for="right_title">Right title</label>
                    <input id="right_title" name="right_title" value="{{ old('right_title', $right['title'] ?? '') }}" placeholder="Optional">
                </div>
                <div>
                    <label for="left_description">Left description</label>
                    <textarea id="left_description" name="left_description" rows="6" placeholder="...">{{ old('left_description', $left['description'] ?? '') }}</textarea>
                </div>
                <div>
                    <label for="right_description">Right description</label>
                    <textarea id="right_description" name="right_description" rows="6" placeholder="...">{{ old('right_description', $right['description'] ?? '') }}</textarea>
                </div>
            </div>

        @elseif ($formType === 'image')
