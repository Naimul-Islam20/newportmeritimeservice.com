@extends('layouts.admin', ['title' => 'Create Award sub-menu'])
...
<div class="card">
    <form method="POST" action="{{ route('admin.award-sub-menus.store') }}">
        @csrf
        ...
        <div class="grid grid-2">
            <div>
                ...
                <input id="label" name="label" value="{{ old('label') }}" required placeholder="e.g. Honorable Client">
                @error('label') <div class="error">{{ $message }}</div> @enderror
            </div>
            <div>
                <label for="url">URL (optional)</label>
                <input id="url" name="url" value="{{ old('url') }}" placeholder="e.g. /award/honorable-client">
                @error('url') <div class="error">{{ $message }}</div> @enderror
                <div style="color:#64748b;font-size:12px;margin-top:6px;">Leave blank to auto-generate under /award/…</div>
            </div>
            <div>
