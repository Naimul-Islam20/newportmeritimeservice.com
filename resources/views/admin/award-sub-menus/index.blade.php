@extends('layouts.admin', ['title' => 'Award — menu'])

@section('content')
<div class="header">
    <h1>Award — sub menus</h1>
    @can('create', \App\Models\SubMenu::class)
    <a href="{{ route('admin.award-sub-menus.create') }}" class="btn btn-primary">Create</a>
    @endcan
</div>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="actions-cell">
                        @can('update', $sub)
                        <form method="POST" action="{{ route('admin.award-sub-menus.toggle-active', $sub) }}" style="display:inline;">
                            @csrf
                            @method('PATCH')
                        ...
                        @endcan
                        @can('delete', $sub)
                        <form method="POST" action="{{ route('admin.award-sub-menus.destroy', $sub) }}" style="display:inline;">
                            @csrf
                            @method('DELETE')
                ...
                @empty
                <tr>
                    <td colspan="6">No sub-menus yet. <a href="{{ route('admin.award-sub-menus.create') }}">Create one</a>.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
