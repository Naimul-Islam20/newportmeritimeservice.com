        ...
        @if ($vfHasHeader)
            <div class="mb-12 text-center">
                @if (filled($visualFrames['mini_title'] ?? null))
                    <h3 class="text-sm font-bold uppercase tracking-wider text-primary">{{ $visualFrames['mini_title'] }}</h3>
                @endif
                @if (filled($visualFrames['title'] ?? null))
                    <h2 class="mt-2 font-sans text-4xl font-bold text-secondary sm:text-5xl lg:text-6xl">{{ $visualFrames['title'] }}</h2>
                @endif
                ...
                @if (filled($visualFrames['description'] ?? null))
                    <p class="mx-auto mt-6 max-w-2xl text-base leading-relaxed text-foreground/70 sm:mt-8 sm:text-lg lg:mb-14">
                ...
                @foreach (array_chunk($tiles, 3) as $rowTiles)
                    <div class="grid grid-cols-1 items-start gap-4 sm:grid-cols-2 sm:gap-5 lg:grid-cols-3 lg:gap-6">
                        @foreach ($rowTiles as $tile)
                            <article class="flex w-full flex-col overflow-hidden rounded-xl bg-foreground/10 shadow-lg shadow-secondary/10">
                                <div class="w-full overflow-hidden rounded-t-xl bg-foreground/5 {{ filled($tile['caption'] ?? null) ? '' : 'rounded-b-xl' }}">
                                    @php($lbSrc = $tile['src'])
                                ...
                                @if (filled($tile['caption'] ?? null))
                                    <h3 class="rounded-b-xl bg-foreground/5 px-3 py-4 text-center font-sans text-lg font-bold leading-snug text-secondary sm:text-xl sm:py-4 lg:text-2xl">{{ $tile['caption'] }}</h3>
                                @endif
                            </article>
                        @endforeach
                    </div>
