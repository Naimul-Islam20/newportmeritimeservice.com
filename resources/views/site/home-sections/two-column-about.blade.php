        ...
        <div class="site-container">
            <div class="grid items-center gap-12 lg:grid-cols-2 lg:gap-16">
                ...
                <div class="flex flex-col justify-center py-6">
                    <h3 class="text-sm font-bold uppercase tracking-wider {{ $stripMiniClass }}">{{ $section->mini_title ?: 'Why Choose Us?' }}</h3>
                    <h2 class="mt-4 font-sans text-4xl font-bold leading-tight {{ $stripTitleClass }} sm:text-5xl lg:text-[3.25rem]">
                    ...
                    @if (count($points) > 0)
                        <ul class="mt-8 max-w-2xl list-disc space-y-4 pl-5 text-base leading-relaxed {{ $stripBodyClass }}">
                            @foreach ($points as $p)
                                <li>{{ $p }}</li>
                            @endforeach
                        </ul>
                    @elseif (filled($section->description))
                        <p class="mt-8 max-w-2xl text-base leading-relaxed {{ $stripBodyClass }}">
                            {{ $section->description }}
                        </p>
                    @endif

                    @if (filled($section->button_label))
                        <a href="{{ $section->resolvedButtonHref() }}" class="mt-10 inline-flex w-fit shrink-0 self-start items-center justify-center rounded-sm bg-primary px-8 py-3.5 text-xs font-bold uppercase tracking-widest text-secondary shadow-md transition-all hover:brightness-95 hover:shadow-lg sm:px-10 sm:py-4">
...
@else
    <section id="about" class="{{ $stripSectionClass }} py-16 sm:py-24">
        <div class="grid lg:grid-cols-2">
            ...
            <div class="flex flex-col justify-start pb-12 pt-12 px-8 sm:p-12 lg:p-16 lg:pt-20 xl:p-24">
                <div class="max-w-xl">
                    <h3 class="text-xs font-bold uppercase tracking-[0.2em] {{ $stripMiniClass }}">{{ $section->mini_title ?: 'About Us' }}</h3>
                    <h2 class="mt-4 font-sans text-3xl font-bold leading-tight {{ $stripTitleClass }} sm:text-4xl lg:text-[2.6rem]">
                        {{ $section->title ?: 'Built on Trust. Driven by Excellence.' }}
                    </h2>

                    <p class="mt-6 text-[0.95rem] leading-relaxed {{ $stripBodyClass }}">
                    ...
                    <p class="mt-4 text-[0.95rem] leading-relaxed {{ $stripBodyClass }}">
